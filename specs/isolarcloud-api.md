...
- **REQ-API-007** — Login attempts MUST try unique `(host, app-key)` pairs in nested order: configured host, `https://augateway.isolarcloud.com`, `https://gateway.isolarcloud.com`, `https://gateway.isolarcloud.eu`, `https://gateway.isolarcloud.com.hk`, `https://gateway.isolarcloud.com.cn`, `https://gateway.isolarcloud.in`; for each host use normalized configured key, current key, old key, then legacy key. Empty pairs and exact duplicates are omitted while preserving first occurrence.
- **REQ-API-008** — A next login candidate MAY be attempted only for a recoverable gateway error. A Docker resolver error involving `127.0.0.11:53` on the first attempted candidate MUST stop rotation and return the Docker-DNS failure category. The same error after an earlier candidate failure MUST stop rotation, retain every attempted failure, and return the recoverable-remote login-sequence category; a later candidate's resolver text MUST NOT reclassify the whole sequence as a Docker-DNS outage.
- **REQ-API-009** — Every unsuccessful sequence containing more than one attempted candidate, including a sequence stopped before exhausting all candidates, MUST be summarized by host in attempt order. The summary MUST identify the first failure and terminal stop reason, include each host's attempt count, retain at most two distinct messages per host, and remain secret-safe. A single failed candidate MUST retain its original classified error.
