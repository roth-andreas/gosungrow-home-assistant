...
- **REQ-ACC-037** — Given a Supervisor-compatible websocket proxy that accepts `/core/websocket` but returns HTTP 403 for `/core/local/...`, and a direct Home Assistant origin that serves the staged `/local/...` asset, installation verifies through the direct origin and succeeds. No static request reaches the Supervisor proxy and no authorization credential reaches the static origin. Given a standalone websocket URL, verification derives its corresponding HTTP origin and still validates status, MIME type, and hash. A genuine direct-origin failure preserves an existing dashboard or produces the native fallback on a fresh installation.
