...
class GoSungrowSourceMappingCard extends HTMLElement {
  ...
  _dialog(metric) {
    ...
    const ranked = [...filtered].sort((left, right) =>
      Number(right.entity_id === metric.recommendation) - Number(left.entity_id === metric.recommendation));
    const recommended = ranked.filter((candidate) => candidate.recommended || candidate.entity_id === metric.recommendation).slice(0, 5);
    ...
    const recommendationVisible = Boolean(metric.recommendation && filtered.some((candidate) => candidate.entity_id === metric.recommendation));
    return `<div class="scrim" data-close="true"><div class="dialog" role="dialog" aria-modal="true" aria-labelledby="source-dialog-title">
      ...
      ${recommendationVisible ? `<div class="recommendation-help"><ha-icon icon="mdi:shield-check-outline"></ha-icon><div><strong>${this._escape(this._label("source_recommended_automatic", "Recommended automatic source"))}</strong><span>${this._escape(this._label("source_recommendation_help", "Select the highlighted source below, then choose Use this source."))}</span></div></div>` : warning ? `<div class="dialog-warning"><ha-icon icon="mdi:alert-circle-outline"></ha-icon><div><strong>${this._escape(this._label("needs_review", "Needs review"))}</strong><span>${this._escape(warning)}</span></div></div>` : ""}
  ...
  _candidate(metric, candidate, peers = this._candidates(metric)) {
    ...
    const recommendedUpdate = metric.recommendation === candidate.entity_id;
    ...
    const accessible = `${recommendedUpdate ? `${this._label("source_recommended_automatic", "Recommended automatic source")}; ` : ""}${friendly}; ${candidate.entity_id}; ${value}`;
    ...
    const reason = recommendedUpdate
      ? this._label("source_recommended_automatic", "Recommended automatic source")
      : candidate.reason || candidate.source || "Compatible entity";
    return `<button class="candidate ${recommendedUpdate ? "recommended-update" : ""} ${selected ? "selected" : ""}" data-select="${this._escape(candidate.entity_id)}" data-metric="${this._escape(metric.key)}" aria-label="${this._escape(accessible)}" ${disabled ? "disabled" : ""}>
      <span class="radio"><span></span></span><span class="candidate-main">${recommendedUpdate ? `<span class="recommendation-badge"><ha-icon icon="mdi:shield-check-outline"></ha-icon>${this._escape(reason)}</span>` : ""}<strong title="${this._escape(friendly)}">${this._escape(displayName)}</strong><code title="${this._escape(candidate.entity_id)}">${this._escape(displayID)}</code><small>${this._escape([candidate.device, candidate.point_id].filter(Boolean).join(" · "))}</small>${recommendedUpdate ? "" : `<small>${this._escape(reason)}</small>`}${semantic ? `<small>${this._escape(semantic)}</small>` : ""}</span><span class="candidate-value">${this._escape(value)}</span>
    </button>`;
  }
  ...
  _styles() { return `
    ...
    .recommendation-help{display:flex;gap:10px;align-items:center;margin:0 clamp(16px,3vw,22px) 14px;padding:11px 13px;border:1px solid color-mix(in srgb,var(--success-color,#22c55e) 48%,transparent);border-radius:12px;background:color-mix(in srgb,var(--success-color,#22c55e) 12%,transparent);color:var(--primary-text-color);font-size:13px}.recommendation-help>ha-icon{color:var(--success-color,#22c55e)}.recommendation-help div{display:grid;gap:2px}.recommendation-help span{color:var(--secondary-text-color)}.candidate.recommended-update.recommended-update{border-color:color-mix(in srgb,var(--success-color,#22c55e) 72%,var(--divider-color));box-shadow:inset 4px 0 var(--success-color,#22c55e);background:color-mix(in srgb,var(--success-color,#22c55e) 9%,transparent)}.candidate.recommended-update.recommended-update:hover,.candidate.recommended-update.recommended-update.selected{border-color:var(--success-color,#22c55e);background:color-mix(in srgb,var(--success-color,#22c55e) 14%,transparent)}.candidate-main .recommendation-badge{display:flex;align-items:center;gap:5px;width:max-content;max-width:100%;margin-bottom:3px;color:var(--success-color,#22c55e);font-size:11px;font-weight:750;line-height:1.25;white-space:normal}.recommendation-badge ha-icon{--mdc-icon-size:16px;flex:0 0 auto}
